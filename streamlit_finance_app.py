...
def fetch_notion_data():
    ...
    for r in resp["results"]:
        ...
        # Expense Category: select → Paid vs Potential
        cat = p.get("Expense Category", {}).get("select", {}).get("name", "")
        is_potential = (cat.lower() == "potential")

        # Calculated Revenue (your new formula field)
...
with tab1:
    ...
    # --- 1) Draw ALL Potential first (hatched outlines) ---
    ...
    # --- 2) Draw ALL Paid on top (solid fill) ---
    ...
    # --- 6) Legends (axis‐level + add_artist) ---
...
with tab2:
    ...
    # Single legend for all four lines
